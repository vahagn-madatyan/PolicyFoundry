...
| D058 | M003-2heki1 | scope | PR fixes as separate milestone from secrets | M003 = 14 PR review fixes, M004 = secrets management | Ship fixes clean on a solid codebase before adding new capability on top | No |
| D059 | M004 | arch | Config merge priority for secrets | init → env vars → .env → keychain → local YAML → global YAML | Explicit env overrides always win; keychain beats config files that might get committed | No |
| D060 | M004 | arch | CLI surface for secret management | `policyfoundry secret set/get/delete` as own subcommand group | Separate from `config` — secrets are a distinct concern from configuration display | No |
| D061 | M004 | library | OS credential store library | `keyring` (jaraco/keyring) as optional dependency | Python standard for credential storage; auto-detects macOS Keychain + Windows Credential Manager; used by gh, twine | No |
| D062 | M004 | scope | Linux credential store support | macOS + Windows only; Linux falls back to .env / env vars | Linux SecretService requires dbus/libsecret system deps — breaks easy install story | Yes — if Linux demand emerges |
